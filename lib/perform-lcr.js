...
async function performLcr(pool, logger, calledNumber) {
  ...
  r2.forEach((o) => {
    ...
    const uri = `sip:${o.e164_leading_plus ? '+' : ''}${calledNumber}@${o.ipv4}:${o.port}`;
    if (!gwMap.has(vc_sid)) gwMap.set(vc_sid, []);
    gwMap.get(vc_sid).push(uri);
  });
...
async function randomizeOutboundGateways(pool, logger, calledNumber) {
  ...
  const uris = shuffle(r.map((o) => `sip:${o.e164_leading_plus ? '+' : ''}${calledNumber}@${o.ipv4}:${o.port}`));
